/**
 * J1939 Fleet Telemetry & Web Dashboard Application
 * Handles Real-Time WebSockets, Fleet State, CAN Sniffer, and Interactive Controls
 */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  let fleet = [];
  let brands = [];
  let selectedVehicleId = 'mb-actros-1851';
  ...
  // CAN Sniffer Rolling Log (Maksimum 100 satır DOM'da tutulur)
  ...
  function renderFleetGrid() {
    ...
    filtered.forEach(v => {
      ...
      // Kart Üzerinden Hız Verme Slider'ı
  ...
  // 4. Canlı Veri Akışında DOM'u Minimum Yükle Güncelle
  ...
  // 5. Araç Seçimi ve Gösterge Paneli / Inspector Güncellemesi
  ...
  function updateSelectedVehicleView() {
    ...
    // 8-Bayt Hex Dump ve SPN 84 Hız Hesabı
  ...
  // 7. Cockpit Arayüzü Olay Dinleyicileri
  ...
  // Hız Hazır Butonları (0, 30, 50, 70, 90, 110, 120 km/h)
  ...
  // Gaz & Fren Pedalları
  const pedalAccEl = document.getElementById('pedalAcc');
  const pedalBrakeEl = document.getElementById('pedalBrake');

  pedalAccEl.addEventListener('click', () => {
    const v = fleet.find(item => item.id === selectedVehicleId);
    if (v) {
      const nextSpeed = Math.min(v.max_speed, v.target_speed + 10);
      speedSliderEl.value = nextSpeed;
      speedSliderValEl.textContent = `${Math.round(nextSpeed)} KM/H`;
      sendSpeedCommand(selectedVehicleId, nextSpeed);
    }
  });

  pedalBrakeEl.addEventListener('click', () => {
    speedSliderEl.value = 0;
    speedSliderValEl.textContent = '0 KM/H';
    sendBrakeCommand(selectedVehicleId, true);
  });

  // Filo Senaryo Butonları
  ...
  // 8. CAN Sniffer / Terminal Yönetimi
